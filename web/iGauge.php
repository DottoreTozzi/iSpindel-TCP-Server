...
if (!$_GET['reset'])
{
 $DataAvailable=isDataAvailable_iGauge($conn, $_GET['name'], $timeFrame);
...
const chartCarb=[<?php echo $angle;?>]
  ...
  $(document).ready(function() 
  {
    ...
    else
    {
        ...
        chart = new Highcharts.Chart(
        {
			...
			const pointData = chartCarb.find(row => row.timestamp === this.point.x)
                        return '<b>' + recipe_name + ' </b>'+pointData.recipe+'<br>'+'<b>'+ this.series.name + ' </b>' + tooltip_at + ' ' + Highcharts.dateFormat('%H:%M', new Date(this.x))  + ' ' + tooltip_time + ' ' + this.y.toFixed(2) +' g/l'; 
                        }else{
                        	const pointData = chartDens.find(row => row.timestamp === this.point.x)
                        return '<b>' + recipe_name + ' </b>'+pointData.recipe+'<br>'+'<b>'+ this.series.name + ' </b>' + tooltip_at + ' ' + Highcharts.dateFormat('%H:%M', new Date(this.x))  + ' ' + tooltip_time + ' ' + this.y.toFixed(2) +' bar';
            ...
            [
                ...
                ,
                {
                    ...
                    yAxis: 2,
                    color: '#00FF00',
                    data: chartCarb.map(row => [row.timestamp, row.value]),
                    marker: 
                        {
                            symbol: 'square',
                            enabled: false,
